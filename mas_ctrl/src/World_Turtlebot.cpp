...
//Triangle
//static double dist = 0.4;
//double dix[TURTLE_NUM]      = {-dist/2,   0,        dist/2};
//double diy[TURTLE_NUM]      = {0,         1.73*dist,  0     };

//Line
static double dist = 1.2;
double dix[TURTLE_NUM]      = {-dist/2,   0,        dist/2};
double diy[TURTLE_NUM]      = {0,         0,  0     };
...
//  ros::Subscriber sub4 = nh.subscribe("/tb3_4/odom", 6, positionCallback4);
  ...
  ros::Publisher pub0 = nh.advertise<geometry_msgs::Twist>("/turtle1/cmd_vel",3);
  ros::Publisher pub1 = nh.advertise<geometry_msgs::Twist>("/turtle2/cmd_vel",3);
  ros::Publisher pub2 = nh.advertise<geometry_msgs::Twist>("/turtle3/cmd_vel",3);
...
//    ROS_INFO("22 Si1 - Sj1: %f %f ## Si1 - Sj2: %f %f",(Si-Sj1)(0),(Si-Sj1)(1),(Si-Sj2)(0),(Si-Sj2)(1));
//    ROS_INFO("Angle 0:%f 1:%f 2:%f",preangle[0],preangle[1],preangle[2]);
    ...
    for (int i=0;i<TURTLE_NUM;i++) {
      if (prelinearv[i]>0 && prelinearv[i]<0.001) {
        prelinearv[i] = 0.001;
      }
      else if(prelinearv[i]<0 && prelinearv[i]>-0.001){
        prelinearv[i] = -0.001;
      }
     if (prelinearv[i]>0.02){
        prelinearv[i] = 0.02;
      }
      else if (prelinearv[i]<-0.02){
        prelinearv[i] = -0.02;
      }
      if (Out0(1)>1.0) Out0(1) =1.0;
      else if (Out0(1)<-1.0) Out0(1) =-1.0;
      if (Out1(1)>1.0) Out1(1) =1.0;
      else if (Out1(1)<-1.0) Out1(1) =-1.0;
      if (Out2(1)>1.0) Out2(1) =1.0;
      else if (Out2(1)<-1.0) Out2(1) =-1.0;
    }
//    ROS_INFO("Angular 0 : %f 1: %f 2: %f",Out0(1),Out1(1),Out2(1));
    ...
    ROS_INFO("1-2 : %f ", sqrt(pow(Turtlepos[0][0]-Turtlepos[1][0],2)+pow(Turtlepos[0][1]-Turtlepos[1][1],2))-dist);
    ROS_INFO("2-3 : %f ", sqrt(pow(Turtlepos[2][0]-Turtlepos[1][0],2)+pow(Turtlepos[2][1]-Turtlepos[1][1],2))-dist);
    ROS_INFO("1-3 : %f ", sqrt(pow(Turtlepos[0][0]-Turtlepos[2][0],2)+pow(Turtlepos[0][1]-Turtlepos[2][1],2))-dist);
